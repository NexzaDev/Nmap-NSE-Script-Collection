...
| KAFKA-AMQP | 🟡 in progress (9 / 16) | 5 CRITICAL/HIGH scripts at spec depth (`kafka-metadata-topic-leak.nse` 2,307, `kafka-unauth-broker-access.nse` 1,724, `kafka-create-topic-allowed.nse` 1,574, `kafka-anonymous-consumer-group.nse` 1,572, `kafka-delete-topic-allowed.nse` 1,556), two MEDIUM (`kafka-plain-auth-over-cleartext.nse` 799, `kafka-sasl-mechanism-audit.nse` 799) and two LOW scripts (`kafka-controller-epoch-leak.nse` 800, `kafka-broker-fingerprint.nse` 753) on top of the 3,161-line `nselib/kafka.lua` wire engine, the 386-line `nselib/tlsprobe.lua` handshake probe and a 1,341-line mock; 9 script suites plus 2 engine unit suites, 95 scenarios, 0 failed assertions |
| LDAP, SMB, RDP, ICS-SCADA, KUBERNETES, SSH, SNMP, NFS-RPC, … | ⬜ not started | still placeholder scripts; see `docs/AUDIT.md` |

## Completed scripts

`node tools/coverage.js` reports 22 scripts meeting both contracts (depth rule and
a wired integration scenario) out of 432: 13 in KERBEROS and 9 in KAFKA-AMQP.
The integration battery is 24 suites and 190 scenarios with 0 failed assertions,
and `node tools/syntax-check.js` compiles 436 files (436 OK, 0 errors) with 129 of
...
| `KAFKA-AMQP/kafka-sasl-mechanism-audit.nse` | 🟡 MEDIUM | 799 | `nselib/kafka.lua` | reads the SASL catalogue and then what the SCRAM exchange itself reveals: every offered mechanism is classified (password-sending / proof-based / delegated / opaque), three server-first messages on their own connections (two names that cannot be accounts and one repeat) expose the salt by length and fingerprint, the salt policy (identical across names, per-account, or changing between two exchanges for one name) and the iteration count graded against the RFC 5802 floor; one exchange is completed per candidate name - capped at four, from `kafka.names` or `kafka.user`, never a built-in wordlist - and its failure token is compared with the token a name that cannot exist receives, which is the enumeration oracle; a supplied credential is completed with a genuine proof whose server signature is verified; the completion probes derive their well-formed proof at one iteration instead of the advertised count so the audit does not pay 100000 rounds, and that limit, the lockout consequence and the PBKDF2 non-attack are stated in the report; 12 integration scenarios |
...
| MEDIUM | 500–800 lines | 6 (`kerberos-kpasswd-service.nse` 800; `kafka-plain-auth-over-cleartext.nse` 799; `kafka-sasl-mechanism-audit.nse` 799; `kerberos-time-skew-audit.nse` 796; `kerberos-preauth-required.nse` 786; `kerberos-fast-negotiation.nse` 730) | 124 |
...
410 of the 432 scripts in the tree breach the depth rule for their class
(`node tools/syntax-check.js --depth`, exit 1). The twenty-two that do not are the
