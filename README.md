...
| KAFKA-AMQP | `kafka-metadata-topic-leak.nse` | 🔴 CRITICAL | real Metadata census: null-topic listing, per-name re-query for the authorization boundary, a generated-name probe with `allow_auto_topic_creation=false` that tells an existence oracle from an authorization-first refusal, DescribeConfigs with byte/duration rendering and sensitive-value detection, DescribeCluster; findings for the inventory, internal-topic exposure (with the meaning of each internal topic), configuration and secret disclosure, sensitive names, replica topology, topic ids, operation masks, listing-vs-ACL inconsistency, under-replication/offline/ISR-less partitions and leader concentration | 13 scenarios |
